...
export class EnemyInfoPanel {
  ...
  show(info: ReturnType<import('../entities/Enemy').Enemy['getInfoSnapshot']>) {
    ...
    // Background ribbon
    const bg = add(this.scene.add.graphics().setDepth(20));
    ...
    const barGfx = add(this.scene.add.graphics().setDepth(20));
    ...
    const addDivider = (x: number) => {
      const g = add(this.scene.add.graphics().setDepth(20));
